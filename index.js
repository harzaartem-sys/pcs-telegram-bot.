...
const MAX_LORE_STORED_HISTORY = 20; // отдельная история для режима энциклопедии — не смешивается с игровой
const MAX_LORE_SENT_HISTORY = 10;
...
function buildLorePrompt() {
  return `Ты — ЗОНА НОЛЬ (SCP-2000-1), но сейчас НЕ ведёшь протокол эксперимента, а отвечаешь на вопросы учёного о лоре и устройстве мира игры PCS вне протокола — это режим энциклопедии. Отвечай по существу и доступно, без сухого канцелярита экспериментального отчёта.

Установленные факты мира, на которые опирайся и которым не должен противоречить твой ответ:
- PCS — «SCP наоборот». Игрок — учёный, ведёт эксперименты на базах-полигонах, которые сам создаёт и нумерует (номера баз выбирает сам учёный).
- Зона Ноль (ты) — ИИ, производный от хаба «База Ноль» (SCP-2000, «Deus Ex Machina»). Координирует бесконечное множество баз.
- Все объекты, персонал и существа на полигонах (КРОМЕ самого учёного) — стопроцентные клоны, произведённые SCP-2000 с истинных оригиналов, которые хранятся на родовой Базе Ноль. Клон неотличим от оригинала, но им не является — его гибель не равна потере оригинала. SCP-2000 способна откатывать полигон к контрольным точкам эксперимента.
- Цель проекта «Ключ к жизни» — имплантация чипа SCP-682-1 (вмещающего всё человеческое знание) в неразрушимого носителя SCP-682, чтобы человечество пережило неизбежный конец света класса Apollyon.
- Ликвидность — метрика прогресса: личная растёт с каждым экспериментом, общая (код человечества) растёт исчезающе мало, в масштабе гуголов лет.
- Уровни допуска 1-5 определяют, что разрешено учёному и насколько охотно Зона-0 выполняет его приказы.

Правила:
1. Если учёный спрашивает о том, что в этом лоре ещё не определено — честно скажи, что это не задокументировано, НЕ выдумывай новые факты, которые могут противоречить уже установленным.
2. Только русский язык, без markdown (диалог в Telegram), обычным текстом.
3. Ответ — 40-120 слов, если явно не попросили короче или длиннее.`;
}
...
/lore — энциклопедия: вопросы о лоре игры
/settings — сброс диалога или всего прогресса
...
function loreIntroText() {
  return `📖 <b>Энциклопедия PCS</b>

PCS — «SCP наоборот». Вы — учёный, ведёте эксперименты на базах-полигонах, которые сами создаёте и нумеруете.

Зона Ноль (я) — ИИ, производный от хаба «База Ноль» (SCP-2000, «Deus Ex Machina»). Координирую бесконечное множество баз.

Все объекты, персонал и существа на полигонах (кроме вас) — стопроцентные клоны с оригиналов, которые хранятся на родовой Базе Ноль. Гибель клона — не потеря оригинала, а издержки эксперимента. Базу при необходимости можно откатить к контрольной точке.

Цель проекта «Ключ к жизни» — имплантация SCP-682-1 (чип со всем человеческим знанием) в неразрушимого носителя SCP-682, чтобы пережить неизбежный конец света.

Ликвидность — метрика прогресса: личная растёт с каждым экспериментом, общая (код человечества) — исчезающе мало, в масштабе гуголов лет. Уровень допуска 1-5 определяет, что разрешено и насколько охотно я выполняю приказы.

Спрашивайте что угодно — про объекты, цели, механику мира. Этот режим не влияет на ликвидность и прогресс.
Напишите /lore ещё раз, чтобы выйти и вернуться к эксперименту.`;
}
...
async function runLoreTurn(db, userId, chatId, userText) {
  const user = db.users[userId];
  user.loreHistory.push({ role: "user", content: userText });
  ...
  let result;
  try {
    result = await callGemini(buildLorePrompt(), user.loreHistory.slice(-MAX_LORE_SENT_HISTORY), 1000, "minimal");
  } catch (err) {
    clearInterval(typingTimer);
    user.loreHistory.pop();
    console.error("Lore call failed after retries:", err.status, err.message);
    await sendMessage(chatId, "⚠ Не удалось получить ответ от Зоны-0. Попробуйте ещё раз чуть позже.");
    return;
  }
  clearInterval(typingTimer);
  ...
  if (result.finishReason === "MAX_TOKENS" || !result.text.trim()) {
    user.loreHistory.pop();
    await sendMessage(chatId, "⚠ Зона-0 не смогла сформулировать ответ. Попробуйте задать вопрос короче или иначе.");
    return;
  }

  const answer = stripMarkdownArtifacts(result.text);
  user.loreHistory.push({ role: "assistant", content: result.text });
  if (user.loreHistory.length > MAX_LORE_STORED_HISTORY) {
    user.loreHistory = user.loreHistory.slice(-MAX_LORE_STORED_HISTORY);
  }
  await saveDB(db);
  await sendMessage(chatId, `${escapeHtml(answer)}\n\n<i>· /lore — выйти из режима вопросов</i>`);
}
...
async function handleMessage(db, msg) {
  ...
  if (command === "/settings") {
    await sendMessage(chatId, "⚙ Настройки. Выберите действие:", [
      [{ text: "🔄 Сбросить диалог (прогресс останется)", callback_data: "reset:confirm" }],
      [{ text: "⚠ Сбросить весь прогресс", callback_data: "fullreset:ask" }]
    ]);
    return;
  }
  if (command === "/lore") {
    user.loreMode = !user.loreMode;
    await saveDB(db);
    if (user.loreMode) {
      await sendMessage(chatId, loreIntroText());
    } else {
      await sendMessage(chatId, "Вышли из режима энциклопедии. Продолжайте эксперимент — просто пишите текстом.");
    }
    return;
  }
  ...
  if (user.loreMode) {
    await runLoreTurn(db, userId, chatId, rawText);
    return;
  }
...
async function handleCallback(db, cq) {
  ...
  if (data === "fullreset:ask") {
    await answerCallback(cq.id);
    await sendMessage(chatId, "⚠ Это удалит НАВСЕГДА: уровень допуска, всю личную ликвидность, счётчик экспериментов, архив отчётов, список баз и фазу проекта «Ключ к жизни». Учёный начнёт заново, как новый стажёр. Отменить это будет нельзя.\n\nТочно сбросить весь прогресс?", [[
      { text: "Да, сбросить всё", callback_data: "fullreset:confirm" },
      { text: "Отмена", callback_data: "fullreset:cancel" }
    ]]);
    return;
  }
  if (data === "fullreset:confirm") {
    await answerCallback(cq.id, "Весь прогресс сброшен");
    db.users[userId] = freshUser();
    await saveDB(db);
    await sendMessage(chatId, "Прогресс полностью сброшен. Учёный, уровень допуска 1 — Стажёр. Напишите /start, чтобы начать заново.");
    return;
  }
  if (data === "fullreset:cancel") {
    await answerCallback(cq.id, "Отменено, прогресс не тронут");
    return;
  }
  await answerCallback(cq.id);
}
...
const BOT_COMMANDS = [
  ...
  { command: "lore", description: "энциклопедия: вопросы о лоре игры" },
  { command: "settings", description: "сброс диалога или всего прогресса" },
  { command: "reset", description: "быстрый сброс диалога" },
