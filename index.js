...
async function callGemini(system, messages, maxTokens, attempts = 3) {
  ...
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      if (finishReason && finishReason !== "STOP") {
        ...
        // дописала ответ (см. комментарий про thinkingConfig выше). Логируем явно, чтобы
        // при повторе не пришлось гадать по обрубку текста, который дошёл до Telegram.
        console.error(`Gemini finishReason=${finishReason}, thoughtsTokenCount=${data.usageMetadata?.thoughtsTokenCount ?? "?"}`);
      }
...
async function runTurn(db, userId, chatId, userText) {
  ...
  let rawText;
  try {
    rawText = await callGemini(buildSystemPrompt(level), user.history.slice(-MAX_SENT_HISTORY), 900);
  } catch (err) {
  ...
  // Лог сырого ответа модели — без него нештатный/обрубленный вывод (см. парсинг ниже)
  // невозможно потом расследовать: видно только то, что дошло до Telegram, а не то, что
  // реально сказала Gemini. Смотреть в Render → Logs.
  ...
  if (newPhase > oldPhase) {
    try {
      const protocolRaw = await callGemini(buildProtocolPrompt(), [{ role: "user", content: "Придумай протокол." }], 300);
...
async function handleMessage(db, msg) {
  ...
  if (command === "/podslushano") {
    ...
    try {
      ...
      const line = await callGemini(buildFeedPrompt(role), [{ role: "user", content: "Реплика." }], 200);
